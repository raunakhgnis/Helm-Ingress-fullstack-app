...
// Read API base URL exclusively from Vite env. Fail fast if it's not set so builds
// and runtime code cannot accidentally call a wrong or missing backend URL.
const API_URL = import.meta.env.VITE_API_URL;
if (!API_URL) {
    // Clear, actionable message for developers/build systems
    console.error('VITE_API_URL is not set. Please add VITE_API_URL to frontend/.env (e.g. VITE_API_URL=http://localhost:30025/api)');
    throw new Error('VITE_API_URL (frontend API base URL) is required but not set');
}
